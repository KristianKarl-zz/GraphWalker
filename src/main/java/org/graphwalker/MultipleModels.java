...
/**
 * A class that handles multiple instances of ModelBasedTesting models and can execute them in
 * parallel. This is only usable for running online since multiple models will be executed at the
 * same time. The method is to add a model and then execute it when appropriate.
 * 
 * @author Ola Sundin
...
public class MultipleModels {
  ...
  /**
   * A static method of getting the only instance of the class. If the instance is not initialized a
  ...
  /**
   * Creating new lists for the models. If an instance of this class is to be 're-used', it may need
   * to clear the lists calling this method.
   */
  public void reset() {
    models = new HashMap<String, ModelBasedTesting>();
    executingModels = new HashMap<String, ModelBasedTesting>();
  ...
  /**
   * Returns the ModelBasedTesting object for the submitted key. If not such key was found null is
   * returned.
   * 
   * @param modelName the key which corresponds to the wanted model
   * @return the ModelBasedTesting object for that key or null if key was not found
   */
  ...
  /**
   * Adds a new model to the set of models. If the submitted name already exists in the set an
   * exception is thrown. In order to guarantee uniqueness of the model name use the helper function
  ...
  /**
   * Returns the statistics from all models. This methods is blocking and will wait until all models
  ...
  /**
   * Helper method to create unique names in a Set of keys. The format of the names will be
   * <i>&lt;name&gt;_&lt;index&gt;</i>. The first name will be the name only and the second will get
   * the index 1.
   * 
   * @param desiredName the desired name
   * @param existingKeys the set of keys that should contain the name
   * @return the actual name that is unique, could be the same as the desired name
   */
